...
if authentication_status:
    ...
    for_refrence = load_file("./simulation_3/for_refrence.csv")
    ...
    image_list = img2list("./simulation_3/split_photo")
    sim2_plants = img2list("./simulation_3/plants")
    sim2_animal = img2list("./simulation_3/animals")



    # if the real_img list not in the sessions state then create a new list with the pictures from the split photo encoded
    if 'real_img_list3' not in st.session_state:
        st.session_state['real_img_list3'] = []
        for file in image_list:
            with open("./simulation_3/split_photo//" + file, "rb") as image:
                encoded = base64.b64encode(image.read()).decode()
                st.session_state['real_img_list3'].append(f"data:image/jpeg;base64,{encoded}")
    ...
    if st.session_state.list3 is None:
        st.session_state['list3'] = []

    # adding the sim2_animal list of images to the sesssion state
    if 'animals3' not in st.session_state:
        st.session_state['animals3'] = sim2_animal

    # adding the sim2_plants list of images to the sesssion state
    if 'plants3' not in st.session_state:
        st.session_state['plants3'] = sim2_plants
    ...
    # in plants for every image in 'plants3' create two columns one with an expander that contains the image and the other with an add button
    # when the add button is pressed add the plant name to the list of species
    with planet:
            for image in st.session_state['plants3']:
                    col1, col2 = st.columns([3,1])
                    with col1:
                        with st.expander(str(image)[:-4]):
                            image_load('./simulation_3/plants/', image)
                    with col2:
                        if st.button('Add', key= str(image)[:-4]):
                            if len(st.session_state.list3) < 8 :
                                if str(image)[:-4] not in st.session_state.list3:
                                    st.session_state.list3.append(str(image)[:-4])

    # in plants for every image in 'animals3' create two columns one with an expander that contains the image of the animal and the other with an add button
    # when the add button is pressed add the animal name to the list of species
    with animal:
            for image in st.session_state['animals3']:
                    col1, col2 = st.columns([3,1])
                    with col1:
                        with st.expander(str(image)[:-4]):
                            image_load('./simulation_3/animals/', image)
                    with col2:
                        if st.button('Add', key= str(image)[:-4]):
                            if len(st.session_state.list3) < 8 :
                                if str(image)[:-4] not in st.session_state.list3:
    ...
    #  the width height and margin are very important in order to have correct image view(calculated by the width of the section divided by the number of columns)
        ...
        st.session_state['real_img_list3'],
            titles=["Elevation:" + str(for_refrence['alt'][i]) +
                    ...
                    "\n" + "Sunlight Hours:" + str(for_refrence['sunlight'][i]) for i in range(len(st.session_state['real_img_list3']))],
    ...
    True_species1 = ['Mountain Wood Fern',
    'Pine Mushroom',
    'Velvet Red Coleus',
    'White Hare',
    'Red Tailed Hawk',
    ...
    True_species2 = ['Mountain Wood Fern',
    'Pine Mushroom',
    'Velvet Red Coleus',
    'Brown Coyote',
    "Townsend's Chipmunk",
    ...
    True_species3 = ['Mountain Wood Fern',
    'Pine Mushroom',
    'Velvet Red Coleus',
    'Brown Coyote',
    "Townsend's Chipmunk",
    ...
    optimal_alt =[681,1000]
    optimal_temp =[28,29.5]
    optimal_wind =[5,25]
    optimal_ph =[4.5,6]
        ...
        for i in range(8):
                ...
                with col4:
                    if st.button('Del', key = str(i) + "del"):
                        if i < len(st.session_state.list3):
                            st.session_state.list3.remove(st.session_state.list3[i])
                with col3:
                    try:
                        st.markdown(st.session_state.list3[i])
                    except IndexError:
                        continue
        with selected:
            Selected = st.markdown("<h5 style='text-align: left; color: grey;'>Selected Species " + str(len(st.session_state.list3)) + "/8</h5>", unsafe_allow_html=True)
        st.markdown('---')
